...
export async function notifyDeliveryBoyNewOrder(order, deliveryPartnerId) {
  ...
  try {
    ...
    let estimatedEarnings = await calculateEstimatedEarnings(deliveryDistance || 0, order);
    ...
    if (notificationSent) {
      ...
      const fcmTokens = [];
      if (deliveryPartner?.fcmToken) fcmTokens.push({ token: deliveryPartner.fcmToken, platform: 'web' });
      if (deliveryPartner?.fcmTokenMobile) fcmTokens.push({ token: deliveryPartner.fcmTokenMobile, platform: 'app' });
      ...
      for (const { token, platform } of fcmTokens) {
        notificationService.sendPushNotification(
          token,
          {
            ...
            body: `New order ${order.orderId} from ${order.restaurantName} (Earnings: ₹${typeof estimatedEarnings === 'object' ? estimatedEarnings.totalEarning : estimatedEarnings})`
          },
          {
            orderId: order.orderId,
            type: 'new_order',
            click_action: '/delivery/orders'
          },
          deliveryPartner.platform || platform || 'web'
        ).catch(err => console.error(`Error sending push notification to delivery boy (${platform}):`, err));
      }
    } else {
...
/**
 * Build the same payload as delivery `new_order_available` (used by restaurant new_order too).
 * @param {Object} order - Order document or lean object (must have _id)
 * @param {string} phase - e.g. 'priority' | 'expanded'
 * @param {string} [paymentMethodOverride] - optional resolved payment method for restaurant flows
 */
export async function buildNewOrderAvailablePayload(order, phase = 'priority', paymentMethodOverride) {
  let orderWithUser = order;
  if (order.userId && typeof order.userId === 'object' && order.userId._id) {
    orderWithUser = order;
  } else if (order.userId) {
  ...
  try {
    estimatedEarnings = await calculateEstimatedEarnings(deliveryDistance, orderWithUser);
    const earnedValue = typeof estimatedEarnings === 'object' ? (estimatedEarnings.totalEarning ?? 0) : (Number(estimatedEarnings) || 0);
  ...
  const payMethod = paymentMethodOverride ?? orderWithUser.payment?.method ?? 'cash';
...
export async function notifyMultipleDeliveryBoys(order, deliveryPartnerIds, phase = 'priority') {
  try {
    ...
    const orderNotification = await buildNewOrderAvailablePayload(order, phase);

    console.log(`📤 Notification payload for order ${order.orderId}:`, {
        ...
        for (const { token, plat } of fcmTokens) {
          notificationService.sendPushNotification(
            token,
            {
              ...
              body: `New order ${orderNotification.orderId} available for pickup from ${orderNotification.restaurantName}`
            },
            {
              orderId: orderNotification.orderId,
              type: 'new_order_available',
            ...
            partner.platform || plat || 'web'
          ).catch(err => console.error(`Error sending push notification to partner ${deliveryPartnerId} (${plat}):`, err));
        }
    ...
    for (const { token, plat } of fcmTokens) {
      notificationService.sendPushNotification(
        token,
        {
          ...
          body: `Order ${order.orderId} from ${order.restaurantName} is ready for pickup.`
        },
        {
          orderId: order.orderId,
          type: 'order_ready',
        ...
        partner.platform || plat || 'web'
      ).catch(err => console.error(`Error sending push notification to delivery boy (${plat}):`, err));
    }
 ...
 */
async function calculateEstimatedEarnings(deliveryDistance, order = null) {
  try {
    ...
    if (!deliveryDistance || deliveryDistance <= 0) {
      ...
      const basePayoutValue = commissionResult.breakdown.basePayout;
      const tipAmount = Number(order?.customerTip) || 0;
      const totalPlusTip = basePayoutValue + tipAmount;
      
      return {
        basePayout: basePayoutValue,
        distance: 0,
        commissionPerKm: commissionResult.breakdown.commissionPerKm,
        distanceCommission: 0,
        customerTip: tipAmount,
        totalEarning: totalPlusTip, // Base payout + tip when distance is 0
        breakdown: `Base payout: ₹${basePayoutValue}${tipAmount > 0 ? ` + Tip: ₹${tipAmount}` : ''}`,
    ...
    if (tipAmount > 0) {
      breakdown += ` + Tip: ₹${tipAmount}`;
    }
    
    breakdown += ` = ₹${totalPlusTip.toFixed(0)}`;
