...
import { MapPin, Search, Mic, SlidersHorizontal, Star, Clock, Bookmark, ArrowRight, Timer, IndianRupee, UtensilsCrossed } from "lucide-react"
...
export default function Dining() {
  ...
  const [diningHeroBanners, setDiningHeroBanners] = useState([diningBanner])
  ...
  // Fetch all dining data dynamically
  useEffect(() => {
    // Wait for zone detection to complete before fetching
    // This prevents stale/wrong zone data from being used
    ...
    const fetchDiningData = async () => {
      try {
        ...
        console.log('[Dining] Fetching with params:', params)

        // 1. Fetch Categories
        const resCategories = await diningAPI.getCategories()
        ...
        // 3. Fetch Must Tries
        const resMustTries = await diningAPI.getMustTries()
        if (resMustTries.data?.success) {
          setMustTryItems(resMustTries.data.data || [])
        }

        // 4. Fetch Restaurants
        const resRestaurants = await diningAPI.getRestaurants(params)
        if (resRestaurants.data?.success) {
          setRestaurantList(resRestaurants.data.data || [])
        }
      } catch (error) {
  ...
  // Auto-play carousel for limelight
  useEffect(() => {
    ...
    <AnimatedPage className="bg-white dark:bg-[#0a0a0a]" style={{ minHeight: '100vh', paddingBottom: '80px', overflow: 'visible' }}>
      ...
      <div className="sticky top-0 z-40 w-full bg-white dark:bg-[#0a0a0a] shadow-sm md:hidden">
        <div className="relative z-20 pt-2 sm:pt-3 lg:pt-4">
          <PageNavbar textColor="dark" zIndex={20} onNavClick={(e) => e.stopPropagation()} />
        </div>

        {/* Search Bar Section */}
        <section className="relative z-20 w-full py-3 sm:py-4 md:py-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="w-full relative">
              <div className="relative bg-white dark:bg-[#1a1a1a] rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 p-1 sm:p-1.5 transition-all duration-300 hover:shadow-xl">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <button type="button" onClick={handleSearchFocus} className="flex-shrink-0 mr-2 sm:mr-3 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
      ...
      {/* Banner Section */}
      <div className="relative w-full px-3 sm:px-4 md:px-6 lg:px-8 pb-4 sm:pb-6 cursor-pointer" onClick={() => navigate('/user/dining/restaurants')}>
              ...
              {diningHeroBanners.map((_, index) => (
                <button
                  ...
                  className={`h-1.5 md:h-2 rounded-full transition-all duration-300 ${
                    index === currentBannerIndex ? 'w-4 md:w-6 bg-white' : 'w-1.5 md:w-2 bg-white/50 hover:bg-white/75'
                  }`}
                />
              ))}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-6 sm:pt-8 md:pt-10 lg:pt-12 pb-6 md:pb-8 lg:pb-10">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {categories.map((category, index) => (
                <Link key={category._id || category.id} to={`/user/dining/${category.name.toLowerCase().replace(/\s+/g, '-')}`}>
                  <motion.div
        ...
        {/* Limelight Carousel Section */}
        {limelightItems.length > 0 && (
            ...
            <div className="relative w-full h-[200px] sm:h-[280px] md:h-[350px] lg:h-[400px] rounded-2xl overflow-hidden shadow-lg">
              ...
              >
                {limelightItems.map((restaurant, index) => (
                  <div key={restaurant._id || restaurant.id} className="min-w-full h-full relative flex-shrink-0 w-full">
                    <OptimizedImage
                      src={restaurant.imageUrl || restaurant.image || diningBanner}
                      alt={restaurant.tagline || "Limelight"}
                      className="w-full h-full"
                      objectFit="cover"
                      sizes="100vw"
                      priority={index === 0}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 z-10 flex flex-col items-start gap-2">
                      {restaurant.percentageOff && (
                        <div className="bg-[#EB590E] text-white px-3 py-1 rounded-full shadow-lg mb-1">
                          <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                            {restaurant.percentageOff}
                          </span>
                        </div>
                      )}
                      <h4 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight drop-shadow-lg">
                        {restaurant.restaurant?.name || restaurant.title || "Special Offer"}
                      </h4>
                      {restaurant.tagline && (
                        <p className="text-sm sm:text-base font-medium text-gray-200 line-clamp-1 max-w-[90%]">
                          {restaurant.tagline}
                        </p>
                      )}
                    </div>
                  ...
                  {limelightItems.map((_, index) => (
                    <button
                      ...
                      className={`h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full transition-all ${
                        index === currentRestaurantIndex ? "bg-white w-6 sm:w-8" : "bg-white/50"
                      }`}
                    />
                  ))}
                </div>
        ...
        {mustTryItems.length > 0 && (
          <div className="mb-6 mt-8 sm:mt-12">
            ...
            <div className="overflow-x-auto -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              <div className="flex gap-4 pb-4" style={{ width: 'max-content' }}>
                {mustTryItems.map((item, index) => (
                  <motion.div
                    ...
                    style={{ width: 'calc((100vw - 3rem) / 2.5)', minWidth: '140px', maxWidth: '200px' }}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.4, delay: index * 0.05 }}
        ...
        <div className="mb-6 mt-8 sm:mt-12">
          ...
          <section className="py-1 mb-4">
            <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-hide pb-1" style={{ scrollbarWidth: "none" }}>
              <Button
                variant="outline"
          ...
          {/* Restaurant List with Menu Scroll */}
          <div className="space-y-10 md:space-y-14">
            {fetching ? (
              <div className="py-20 text-center text-gray-500 font-bold">Loading dynamic restaurants...</div>
                ...
                return (
                  <div key={restaurant._id || restaurant.id} className="space-y-4">
                    ...
                    <div className="relative">
                      <div className="flex overflow-x-auto pb-4 gap-3 sm:gap-4 scrollbar-hide scroll-smooth" style={{ scrollbarWidth: 'none' }}>
                    ...
                    <Link className="flex justify-start" to={`/dining/${restaurant.diningSettings?.diningType || 'family-dining'}/${restaurantSlug}`}>
                      <Button variant="outline" className="rounded-lg bg-gray-50 dark:bg-[#1a1a1a] hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-white text-gray-700 border-gray-200 dark:border-gray-800 h-9 md:h-10 px-4 md:px-6 text-sm md:text-base">
              ...
              <div className="py-20 text-center text-gray-400 font-bold">No dining places found matching this location.</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100]" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:max-w-4xl bg-white dark:bg-[#1a1a1a] rounded-t-3xl md:rounded-3xl max-h-[85vh] md:max-h-[90vh] flex flex-col">
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 py-4 px-2 text-center relative transition-colors ${
                        isActive ? 'bg-white dark:bg-[#1a1a1a] text-[#EB590E]' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'
                      }`}
                    >
                      {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#EB590E] rounded-r" />}
              ...
              <div ref={rightContentRef} className="flex-1 overflow-y-auto p-4 md:p-6">
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">Under 30 mins</span>
                      </button>
                      <button
                      ...
                      >
                        <Timer className="h-6 w-6 text-gray-600" />
                        <span className="text-sm font-medium">Under 45 mins</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {activeFilterTab === 'rating' && (
                  <div className="space-y-4 mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {['rating-35-plus', 'rating-4-plus', 'rating-45-plus'].map((rid, rIdx) => {
                        const labels = ['3.5+ Rating', '4.0+ Rating', '4.5+ Rating']
                        return (
                          <button
                            key={rid}
                            onClick={() => toggleFilter(rid)}
                            className={`flex flex-col items-center gap-2 p-4 rounded-xl border transition-colors ${
                              activeFilters.has(rid)
                                ? 'border-[#EB590E] bg-[#FFF2EB] dark:bg-[#EB590E]/20'
                                : 'border-gray-200 dark:border-gray-700 hover:border-[#EB590E]'
                            }`}
                          >
                            <Star className="h-6 w-6 text-yellow-500 fill-yellow-500" />
                            <span className="text-sm font-medium">{labels[rIdx]}</span>
                          </button>
                        )
                      })}
                    </div>
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">Under 1 km</span>
                      </button>
                      <button
                      ...
                      >
                        <MapPin className="h-6 w-6 text-gray-600" />
                        <span className="text-sm font-medium">Under 2 km</span>
                      </button>
                    </div>
                  </div>
                )}
            ...
            <div className="flex items-center gap-4 md:gap-6 px-4 md:px-6 py-4 md:py-5 border-t dark:border-gray-800 bg-white dark:bg-[#1a1a1a]">
              <button onClick={() => setIsFilterOpen(false)} className="flex-1 py-3 md:py-4 text-center font-semibold text-gray-700 dark:text-gray-300 text-sm md:text-base">
                Close
              </button>
              <button
                onClick={() => setIsFilterOpen(false)}
                className="flex-1 py-3 md:py-4 font-semibold bg-[#EB590E] text-white hover:bg-[#D94F0C] rounded-xl transition-colors text-sm md:text-base"
              >
                Show Results
              </button>
            </div>
