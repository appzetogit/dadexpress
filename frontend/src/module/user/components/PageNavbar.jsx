...
export default function PageNavbar({
  ...
  const locationDisplay = (() => {
    ...
    if (location?.mainTitle &&
      ...
      false && console.log("✅✅✅ ZOMATO-STYLE: Using mainTitle for display:", mainLocation);
    }
    ...
    if (!mainLocation && location?.formattedAddress && !isCoordinates(location.formattedAddress) && location.formattedAddress !== "Select location") {
      ...
      if (formattedParts.length >= 4) {
        false && console.log("🔍 Using formattedAddress (complete address detected):", location.formattedAddress)
        ...
        if (cityIndex > 0) {
          ...
          if (localityParts.length > 0) {
            ...
            false && console.log("✅✅✅ Extracted locality from complete formattedAddress:", mainLocation)
          }
        } else {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            false && console.log("✅✅✅ Using first parts from formattedAddress:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation && location?.address && !isCoordinates(location.address) && location.address !== "Select location") {
      false && console.log("🔍 Processing address field (Priority 2):", location.address)
      ...
      if (!cityInAddress && addressParts.length > 0) {
        ...
        false && console.log("✅ Using address field as locality:", mainLocation)
      } else {
        ...
        if (cityIndex > 0) {
          ...
          false && console.log("✅ Extracted locality from address field:", mainLocation)
        } else if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          false && console.log("✅ Using first 3 parts from address:", mainLocation)
        }
      }
    }
    ...
    if (!mainLocation && location?.formattedAddress &&
      ...
      location.formattedAddress !== "Select location") {
      false && console.log("🔍 Processing formattedAddress (Priority 2):", location.formattedAddress)
      const parts = location.formattedAddress.split(',').map(part => part.trim()).filter(part => part.length > 0)
      false && console.log("📋 Address parts:", parts)
      ...
      false && console.log("📋 Filtered parts (without pincode/country):", filteredParts)
      ...
      if (location?.city) {
        ...
        false && console.log(`📍 City index (exact match with "${location.city}"):`, cityIndex)
      }
      ...
      if (cityIndex === -1) {
        ...
        false && console.log("📍 City index (common cities):", cityIndex)
      }
      ...
      if (cityIndex === -1 && location?.state) {
        ...
        if (stateIndex > 0) {
          ...
          cityIndex = stateIndex - 1
          false && console.log("📍 City index (before state):", cityIndex)
        }
      }
      ...
      if (cityIndex === -1) {
        ...
        if (stateIndex > 0) {
          cityIndex = stateIndex - 1
          false && console.log("📍 City index (before state name):", cityIndex)
        }
      }
      ...
      if (cityIndex > 0) {
        ...
        if (localityParts.length > 0) {
          mainLocation = localityParts.join(', ')
          false && console.log("✅✅✅ Using exact locality from formattedAddress:", mainLocation)
        } else {
          false && console.warn("⚠️ No locality parts found before city")
        }
      } else {
        ...
        if (stateIndex > 0) {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            false && console.log("✅✅✅ Using parts before state from formattedAddress:", mainLocation)
          }
        }
        ...
        if (!mainLocation && filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          false && console.log("✅✅✅ Using first 3 parts from formattedAddress (fallback):", mainLocation)
        } else if (!mainLocation && filteredParts.length >= 2) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          false && console.log("✅ Using first 2 parts from formattedAddress (fallback):", mainLocation)
          ...
          if (!isCoordinates(firstPart) && firstPart.length > 2) {
            ...
            false && console.log("✅ Using first part from formattedAddress (fallback):", mainLocation)
          }
        }
      }

      if (!mainLocation) {
        false && console.warn("⚠️⚠️⚠️ Could not extract locality from formattedAddress")
      } else {
        false && console.log("🎯🎯🎯 Final mainLocation extracted:", mainLocation)
      }
    }
    ...
    if (!mainLocation &&
      ...
      false && console.log("🔍 Processing address field:", location.address)
      ...
      false && console.log("📋 Filtered parts:", filteredParts)
      ...
      if (filteredParts.length === 0 ||
        ...
        false && console.log("⚠️ Address field only contains placeholder, skipping")
        ...
        if (cityIndex > 0) {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            false && console.log("✅ Using exact locality from address:", mainLocation)
          }
        } else if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          false && console.log("✅ Using first 3 parts from address:", mainLocation)
          ...
          mainLocation = filteredParts.slice(0, 2).join(', ')
          false && console.log("✅ Using first 2 parts from address:", mainLocation)
          ...
          if (!isCoordinates(firstPart) &&
            ...
            mainLocation = firstPart
            false && console.log("✅ Using first part from address:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation) {
      ...
      if (addressToUse &&
        ...
        if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          false && console.log("✅ Using first 3 parts as fallback:", mainLocation)
        } else if (filteredParts.length >= 2) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          false && console.log("✅ Using first 2 parts as fallback:", mainLocation)
        } else if (filteredParts.length >= 1) {
          const firstPart = filteredParts[0]
          if (!isCoordinates(firstPart) && firstPart.length > 2) {
            mainLocation = firstPart
            false && console.log("✅ Using first part as fallback:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation &&
      ...
      !isCoordinates(location.formattedAddress) &&
      location.formattedAddress !== "Select location") {
      false && console.log("🔄🔄🔄 FORCE EXTRACTING from formattedAddress (last resort):", location.formattedAddress)
      ...
      if (filteredParts.length >= 3) {
        ...
        if (thirdPart === "indore" || thirdPart === location?.city?.toLowerCase()) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          false && console.log("✅✅✅ FORCE: Using first 2 parts (3rd is city):", mainLocation)
        } else {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          false && console.log("✅✅✅ FORCE: Using first 3 parts:", mainLocation)
        }
      } else if (filteredParts.length >= 2) {
        mainLocation = filteredParts.slice(0, 2).join(', ')
        false && console.log("✅✅✅ FORCE: Using first 2 parts:", mainLocation)
      } else if (filteredParts.length >= 1) {
        const firstPart = filteredParts[0]
        if (!isCoordinates(firstPart) && firstPart.length > 2 && firstPart.toLowerCase() !== location?.city?.toLowerCase()) {
          mainLocation = firstPart
          false && console.log("✅✅✅ FORCE: Using first part:", mainLocation)
        }
      }
    }
    ...
    if (!mainLocation && location?.area && location.area.trim() !== "" && !isCoordinates(location.area)) {
      ...
      if (location?.city && location.city.trim() !== "" && location.city !== "Unknown City" &&
        ...
        false && console.log("✅ Using area + city:", mainLocation)
      } else {
        mainLocation = location.area
        false && console.log("✅ Using area name:", mainLocation)
      }
    }
    ...
    // Skip if city is "Current Location" or "Select location" - these are placeholders
      ...
      false && console.log("⚠️⚠️⚠️ FALLBACK: Using city (no locality found):", mainLocation)
    }
    ...
    if (isCoordinates(mainLocation)) {
      ...
      false && console.log("⚠️ Replaced coordinates with:", mainLocation)
    }
    ...
    if (mainLocation && (mainLocation.toLowerCase() === location?.city?.toLowerCase() || mainLocation === "Indore")) {
      false && console.log("🔄🔄🔄 MainLocation is city, trying to extract locality one more time...")
      ...
      if (location?.area && location.area.trim() !== "" &&
        ...
        mainLocation = `${location.area}, ${location.city}`
        false && console.log("✅✅✅ Using area from location object:", mainLocation)
        ...
        const parts = location.formattedAddress.split(',').map(p => p.trim()).filter(p => p.length > 0)
        false && console.log("🔍 Extracting area from formattedAddress parts:", parts)
        ...
        if (cityIndex > 0) {
          ...
          if (areaParts.length > 0) {
            ...
            if (extractedArea && extractedArea.toLowerCase() !== location.city.toLowerCase() &&
              ...
              false && console.log("✅✅✅ Extracted area from formattedAddress (before city):", mainLocation)
              ...
              if (lastTwoParts.every(p => p.toLowerCase() !== location.city.toLowerCase())) {
                ...
                false && console.log("✅✅✅ Extracted area (2 parts) from formattedAddress:", mainLocation)
              }
            }
          }
        } else {
    ...
    if (mainLocation && mainLocation.toLowerCase() === location?.city?.toLowerCase()) {
      ...
      if (location?.area && location.area.trim() !== "" &&
        ...
        !isCoordinates(location.area)) {
        mainLocation = `${location.area}, ${location.city}`
        false && console.log("✅✅✅ Added area to city display:", mainLocation)
      }
        ...
        if (parts.length >= 3) {
          ...
          for (let i = parts.length - 3; i >= 0; i--) {
            ...
            if (part && part.toLowerCase() !== location.city.toLowerCase() &&
              ...
              false && console.log("✅✅✅ Last resort: Extracted area from formattedAddress:", mainLocation)
              break
            }
          }
        }
    ...
    if (hasCity && hasState) {
      ...
      if (location?.formattedAddress) {
        ...
        false && console.log("📍 Extracting city/state from formattedAddress:", {
          formattedAddress: location.formattedAddress,
          parts: parts,
        ...
        if (parts.length >= 4) {
          ...
          false && console.log("📍 Pincode index:", pincodeIndex)
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            false && console.log("📍 Extracted from pincode position:", { cityPart, statePart, pincodeIndex })
            ...
            if (cityPart && statePart &&
              ...
              false && console.log("✅ Using extracted city/state (pincode method):", subLocation)
            }
          }
          ...
          if (!subLocation && parts.length >= 4) {
            ...
            false && console.log("📍 Trying India method:", { lastPart, secondLastPart })
            ...
            if (lastPart === "India" && /^\d{6}$/.test(secondLastPart)) {
              ...
              false && console.log("📍 Extracted from India position:", { cityPart, statePart })
              ...
              if (cityPart && statePart &&
                ...
                statePart.length > 2) {
                subLocation = `${cityPart}, ${statePart}`
                false && console.log("✅ Using extracted city/state (India method):", subLocation)
              }
            }
          }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            if (pincodeIndex === 6 && parts.length >= 7) {
              ...
              false && console.log("📍 Direct extraction (index method):", { cityPart, statePart, pincodeIndex })
              ...
              if (cityPart && statePart &&
                ...
                statePart.length > 2) {
                subLocation = `${cityPart}, ${statePart}`
                false && console.log("✅ Using extracted city/state (direct index method):", subLocation)
              }
            }
          }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            false && console.log("📍 Simple fallback (parts[4] and parts[5]):", {
              cityPart,
              statePart,
              partsLength: parts.length,
              allParts: parts
            ...
            if (cityPart && statePart &&
              ...
              statePart.length > 1) {
              subLocation = `${cityPart}, ${statePart}`
              false && console.log("✅ Using extracted city/state (simple fallback):", subLocation)
            } else {
              false && console.log("⚠️ Validation failed for parts[4] and parts[5]:", { cityPart, statePart })
            }
          }
        }
      }
      ...
      if (!subLocation && location?.address && location.address !== location?.formattedAddress) {
        const parts = location.address.split(',').map(part => part.trim()).filter(part => part.length > 0)
        false && console.log("📍 Trying address field:", { address: location.address, parts })
        ...
        if (parts.length >= 4) {
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            if (cityPart && statePart &&
              ...
              statePart.length > 2) {
              subLocation = `${cityPart}, ${statePart}`
              false && console.log("✅ Using extracted city/state from address field:", subLocation)
            }
          }
        }
      }
      ...
      if (!subLocation) {
        subLocation = ""
        false && console.log("⚠️ Could not extract city/state from address")
      }
    }

    // Debug log
    false && console.log("📍 PageNavbar Location Display:", {
      location: location,
      city: location?.city,
      state: location?.state,
      hasCity,
    ...
    if (subLocation && (subLocation.includes("G-2") || subLocation.includes("Princess Center"))) {
      false && console.warn("⚠️⚠️⚠️ WRONG subLocation detected:", subLocation)
      false && console.warn("⚠️ Forcing re-extraction from formattedAddress")
      ...
      if (location?.formattedAddress) {
        ...
        if (parts.length >= 6) {
          ...
          if (cityPart && statePart &&
            ...
            statePart.length > 1) {
            subLocation = `${cityPart}, ${statePart}`
            false && console.log("✅✅✅ FORCED extraction - New subLocation:", subLocation)
          }
        }
      }
    }
