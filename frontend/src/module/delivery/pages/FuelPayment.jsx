...
export default function TipsStatement() {
  ...
  const minimumPocketBalance = 100
  ...
  // Fetch delivery wallet to get real pocket balance for fuel payment eligibility
  useEffect(() => {
    const loadWallet = async () => {
      try {
        ...
        console.error("Error fetching wallet for fuel payment:", error)
        setWalletState(null)
      } finally {
  ...
  const pocketBalance = walletState?.pocketBalance ?? walletState?.totalBalance ?? 0
  const isLowBalance = pocketBalance < minimumPocketBalance
    ...
    <div className="min-h-screen bg-white overflow-x-hidden pb-24 md:pb-6">
      ...
      <div className="flex mt-64 text-center flex-col w-screen h-screen content-center  align-center ">
        ...
        <div className="text-black text-sm font-semibold">
          {isLoadingWallet
            ? "Checking your pocket balance..."
            : isLowBalance
            ? "Your pocket balance is low"
            : "You have enough pocket balance for fuel payments"}
        </div>
        <div className="text-gray-800 text-sm ">
          ...
          {minimumPocketBalance} pocket balance is required to pay for fuel.{" "}
          {!isLoadingWallet && (
            <span className="font-semibold text-black">
              (Current: ₹{Math.round(pocketBalance)})
            </span>
          )}
        </div>
