...
function MetricCard({ title, value, helper, icon, accent, path, isLoading, disableNavigation = false }) {
  ...
  const hardDisableClicks = isPlatformFeeCard
  ...
  return (
    <Card
      className={`overflow-hidden border-neutral-200 bg-white p-0 ${isNavigable ? "cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]" : "cursor-default"} ${hardDisableClicks ? "pointer-events-none" : ""}`}
      onClick={(e) => {
        if (!isNavigable) {
          e.preventDefault()
          e.stopPropagation()
